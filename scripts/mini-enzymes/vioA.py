...
def main(use_modal: bool = False, experiment_name: str | None = None, temperature: float = 1e-4, n_steps: int = 100000):
    """
    PLM-based mini-enzyme optimization for VioA (PDB: 6fw9).

    VioA is a tryptophan 2-monooxygenase involved in violacein biosynthesis.
    Catalytic residues: R64, K269, Y309. FAD-binding residues: S15, D38, R46, L208, A245, M398.
    ...
    full_sequence = get_sequence_from_pdb_id(pdb_id="6fw9", sequence_index=0)

    chain_atoms = bg.oracles.folding.utils.pdb_file_to_atomarray(fetch("6fw9", format="pdb"))
    chain_atoms = get_atomarray_in_residue_range(chain_atoms, start=offset, end=418)

    # Catalytic residues: R64, K269, Y309
    crucial_residues_indices = [64, 269, 309]
    # FAD-binding residues: S15, D38, R46, L208, A245, M398
    crucial_residues_indices += [15, 38, 46, 208, 398, 245]
    crucial_residues_indices = sorted(crucial_residues_indices)
    ...
    expected_crucial_residues = ["Ser15", "Asp38", "Arg46", "Arg64", "Leu208", "Ala245", "Lys269", "Tyr309", "Met398"]
    ...
    # VioA is a dimer — only validate against first half of crucial atom residues
    temp_residues = list(sequence_from_atomarray(crucial_atoms))
    halfway = len(temp_residues) // 2
    crucial_atoms_residues = temp_residues[:halfway]
