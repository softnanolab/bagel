...
def main(use_modal: bool = False, experiment_name: str | None = None, temperature: float = 1e-4, n_steps: int = 100000):
    """
    PLM-based mini-enzyme optimization for PETase (PDB: 5xjh).

    PETase degrades polyethylene terephthalate (PET) plastic. Catalytic triad:
    S160 (nucleophile), D206, H237. Additional functional residues: W185, S214, I218, Y87.
    ...
    log_path = pl.Path(os.environ.get("DATA_DIR", "./logs")) / 'mini-enzymes' / 'plm' / 'petase'

    offset = 30

    full_sequence = get_sequence_from_pdb_id(pdb_id="5xjh", sequence_index=0)

    chain_atoms = bg.oracles.folding.utils.pdb_file_to_atomarray(fetch("5xjh", format="pdb"))
    chain_atoms = get_atomarray_in_residue_range(chain_atoms, start=offset, end=292)

    # Catalytic triad: S160, D206, H237
    # Additional functional residues: W185, S214, I218, Y87
    crucial_residues_indices = [87, 160, 185, 206, 214, 218, 237]
    ...
    expected_crucial_residues = ["Tyr87", "Ser160", "Trp185", "Asp206", "Ser214", "Ile218", "His237"]
