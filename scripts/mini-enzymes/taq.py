...
def main(use_modal: bool = False, experiment_name: str | None = None, temperature: float = 1e-4, n_steps: int = 100000):
    """
    PLM-based mini-enzyme optimization for Taq DNA polymerase (PDB: 1taq).

    Taq polymerase is a thermostable DNA polymerase used in PCR.
    Catalytic residues: D610, F667 (fidelity), D785, E786.
    ...
    full_sequence = get_sequence_from_pdb_id(pdb_id="1taq", sequence_index=0)

    chain_atoms = bg.oracles.folding.utils.pdb_file_to_atomarray(fetch("1taq", format="pdb"))
    chain_atoms = get_atomarray_in_residue_range(chain_atoms, start=offset, end=832)

    # Catalytic residues: D610, F667 (fidelity), D785, E786
    crucial_residues_indices = [610, 667, 785, 786]
    ...
    expected_crucial_residues = ["Asp610", "Phe667", "Asp785", "Glu786"]
