...
def test_remove_residue_from_start_of_chain(short_chain: dp.Chain) -> None:
...
def test_remove_residue_from_end_of_chain(short_chain: dp.Chain) -> None:
    starting_length = len(short_chain.residues)
...
def test_add_residue_at_start_of_chain(short_chain: dp.Chain) -> None:
    starting_length = len(short_chain.residues)
...
def test_add_residue_at_end_of_chain(short_chain: dp.Chain) -> None:
    starting_length = len(short_chain.residues)
...
def test_mutate_residue_at_end_of_chain(short_chain: dp.Chain) -> None:
    starting_length = len(short_chain.residues)
...
def test_create_chain_from_pdb(pdb_path: str) -> None:
    chain = dp.Chain.from_pdb(pdb_path, chain_id='A')
...
def test_custom_chain_ids(base_sequence: str) -> None:
    _ = [dp.Residue(name=aa, chain_ID='1234', index=i, mutable=True) for i, aa in enumerate(base_sequence)]
    with pytest.raises(AssertionError):
        _ = [dp.Residue(name=aa, chain_ID='12345', index=i, mutable=True) for i, aa in enumerate(base_sequence)]
    with pytest.raises(AssertionError):
        _ = [
            dp.Residue(name=aa, chain_ID='TOO_LONG_CHAIN_ID', index=i, mutable=True)
